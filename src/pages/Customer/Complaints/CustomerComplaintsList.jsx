...
const CustomerComplaintsList = () => {
  ...
  const { aadharNumber = 100000000001 } = user || {};
  const [list, setList] = useState([]);

  useEffect(() => {
    axios
      .get(`http://localhost:5000/api/customer/getUserAllComplaints/${aadharNumber}`)
      .then((data) => {
        const list = data.data;
        console.log(list);
        const newList = [];
        list.forEach((element) => {
          const complaintData = element[8];
          newList.push(JSON.parse(complaintData));
        });
        setList(newList);
      });
  }, [aadharNumber]);
  ...
  return (
    <div className="w-full min-h-[20vh] px-4 py-2 pt-4 ">
      ...
      <div
        ...
        {list
          ?.slice()
          .reverse()
          .map((complaintData, id) => (
            <ComplaintCard
              complaintData={complaintData}
              key={id}
              index={list.length - id - 1}
            />
          ))}
      </div>
    </div>
    ...
    //                         <span className="text-2xl font-semibold font-title">
    //                             Dashboard
    //                         </span>
    //                         <svg
    //                             className="w-4 h-4"
